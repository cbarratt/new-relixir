defmodule NewRelixir.Plug.PhoenixControllerTest do
  ...
  test "it assigns a transaction to the connection", %{conn: conn} do
    conn = NewRelixir.Plug.PhoenixController.call(conn, nil)
  ...
  test "if New Relic is not configured, it does not modify connection", %{conn: conn} do
    assert NewRelixir.Plug.PhoenixController.call(conn, nil) == conn
  end
end
